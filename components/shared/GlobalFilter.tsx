...
import {
  ...
  GLOBAL_CONTEXT_ALL_BUSINESSES,
  GLOBAL_CONTEXT_ALL_DIRECTORY,
  GLOBAL_CONTEXT_ALL_EVENTS,
  GLOBAL_CONTEXT_ALL_SERVICES,
  GLOBAL_CONTEXT_STORES,
  GLOBAL_FILTER_SECTORS,
  categoryNamesForSector,
  getContextTabOptions,
  getDefaultContextTab,
  type CategoryRecord,
  type GlobalFilterSector,
} from "@/types/taxonomy";
import { getActivityLabel, getEventSubTypeLabel, type Translations } from "@/types/i18n";

export type GlobalFilterHandler = (
...
export interface GlobalFilterProps {
  ...
  activeSector: GlobalFilterSector;
  activeContextTab: string;
  activeCategory: string;
  onFilterChange: GlobalFilterHandler;
  layout?: "overlay" | "inline" | "stacked";
...
function getSectorLabel(sector: GlobalFilterSector, labels: Translations): string {
  if (sector === "ALL") return labels.all;
  if (sector === "Food") return labels.food;
  if (sector === "Services") return labels.catalogServices;
  return labels.events;
}

function getContextTabLabel(value: string, labels: Translations): string {
  if (value === GLOBAL_CONTEXT_ALL_DIRECTORY) return labels.globalAllDirectory;
  if (value === GLOBAL_CONTEXT_ALL_BUSINESSES) return labels.globalAllBusinesses;
  if (value === GLOBAL_CONTEXT_ALL_SERVICES) return labels.globalAllServicesContext;
  if (value === GLOBAL_CONTEXT_ALL_EVENTS) return labels.globalAllEvents;
  if (value === GLOBAL_CONTEXT_STORES) return labels.directoryEntityStores;
  if (value === GLOBAL_CONTEXT_ACTIVITIES) return labels.directoryEntityActivities;
  return value;
}
...
function getCategoryLabel(
  ...
  if (value === GLOBAL_ALL_CATEGORY) return labels.filterAll;
  if (sector === "Events" || sector === "ALL") {
    return getEventSubTypeLabel(value, labels);
  }
...
export function GlobalFilter({
  ...
  const [openMenu, setOpenMenu] = useState<"context" | "category" | null>(null);

  const contextOptions = useMemo(
    () => getContextTabOptions(activeSector),
    [activeSector],
  );

  const categoryOptions = useMemo(() => {
    const names = categoryNamesForSector(categories, activeSector);
    return [GLOBAL_ALL_CATEGORY, ...names];
  }, [activeSector, categories]);

  const resolvedContext = contextOptions.includes(activeContextTab)
    ? activeContextTab
    : getDefaultContextTab(activeSector);
  ...
  const emitChange = (
    ...
    onFilterChange(sector, contextTab, category);
  };

  const handleSectorChange = (sector: GlobalFilterSector) => {
    emitChange(sector, getDefaultContextTab(sector), GLOBAL_ALL_CATEGORY);
    setOpenMenu(null);
  };

  const handleContextChange = (contextTab: string) => {
    emitChange(activeSector, contextTab, resolvedCategory);
    setOpenMenu(null);
  };

  const handleCategoryChange = (category: string) => {
    emitChange(activeSector, resolvedContext, category);
    setOpenMenu(null);
  };

  const containerClassName =
    layout === "overlay"
      ? "pointer-events-auto absolute top-6 left-1/2 z-20 flex w-[min(100%,22rem)] -translate-x-1/2 flex-col gap-3"
      : layout === "inline"
        ? "flex flex-col gap-3"
        : "flex flex-col gap-3";

  const capsuleLayoutId = `${layoutIdPrefix}-sector-capsule`;

  return (
    <div className={containerClassName}>
      <div
        role="tablist"
        aria-label={labels.globalFilterSector}
        className={`${capsuleShellClassName} hide-scrollbar w-full overflow-x-auto`}
      >
        <div className="flex w-max min-w-full items-center gap-1">
          {GLOBAL_FILTER_SECTORS.map((sector) => {
      ...
      <div className="relative flex gap-2">
        <FilterMenu
          id={contextMenuId}
          label={labels.globalFilterContextTab}
          value={resolvedContext}
          options={contextOptions}
          isOpen={openMenu === "context"}
          onToggle={() =>
            setOpenMenu((current) => (current === "context" ? null : "context"))
          }
          onSelect={handleContextChange}
          getOptionLabel={(option) => getContextTabLabel(option, labels)}
        />

        <FilterMenu
          id={categoryMenuId}
          label={labels.globalFilterCategory}
